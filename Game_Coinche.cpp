...
#else
		if (i % 2 == 0) _players[i] = static_cast<Player*>(new Player_AI<AIGameMemoryImproved, AITakeBasic, AIPlayMonteCarlo<3000,AIGameMemoryImproved, McPlayRandom<AIGameMemoryImproved, 1, AIPlayRandom<Cards_Basic, AIGameMemoryImproved> > > >(Player_ID(posPlayer[i]), screenWidth, screenHeight, _event, &_backSide, _pScreen));
